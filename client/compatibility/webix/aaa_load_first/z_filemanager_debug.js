			...
			oninit: function(){
				if(this.$$("table")){
					...
					this.$$("table").attachEvent("onBeforeEditStart", function(id){
						...
						if(!this._skipEditStart){
							if(typeof(id) == "object")
								return false;
							this._skipEditStart = true;
							this.edit({row:id,column: "value"});
							this._skipEditStart = false;
							return false;
						}
						return true;
					});
...
webix.protoUI({
	...
	_setFSId: function(item){
		this.changeId(item.id,this.getParentId(item.id)+"/"+item.value);
	},
	...
	_makeSaveRequest: function(url,obj,callback){
		if(this.callEvent("onBeforeRequest",[url, obj])){
			...
			webix.ajax().post(url, webix.copy(obj), {
				success: webix.bind(function(text,response){
					var data = this.data.driver.toObject(text, response);
	...
	* */
	...
	_openDynFolder: function(ids, mode, defer){
		...
		this.showProgress();
		webix.ajax().post(this.config.handlers[mode], { action: mode, source: ids[0]}, {
			success: webix.bind(function(text,response){
				this.hideProgress();
				var data = this.data.driver.toObject(text, response);
	...
	loadDynData: function(url, obj, mode, open){
		if(typeof url == "string"){
			this.showProgress();
			if(this.callEvent("onBeforeDynLoad", [url, obj, mode])){
				webix.ajax().post(url, { action: mode, source: obj.id}, {
					success: webix.bind(function(text,response){
						this.hideProgress();
						var data = this.data.driver.toObject(text, response);
						if(open)
